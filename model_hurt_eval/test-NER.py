...
count_misc = 0
total_misc = 0
for j in tqdm(range(len(valid_sentences)), desc=f'{args.task} evaluation'):
    result = open(f"./test-result/test-NER/result-NER-{args.base_model}-{args.eval_name}.txt", "a", encoding="utf8")
    result.write("*******************************************************************" + "\n")
    sentence = ""
    per = []
    org = []
